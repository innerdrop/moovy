...
> **Sesión 2026-07-25/26 (QA founder + rediseño perfil + PWA — resumen)**: **3 ramas cerradas** (mergeadas a develop; deploy `-NoDB` en curso al cierre). (1) `fix/aprobacion-docs-pipeline-y-portada`: aprobar docs desde OPS ahora SÍ habilita al comercio (doc cumplido = valor O status APPROVED; APPROVED global ⇒ docs OK; `toggleMerchantOpen` tenía una 3ª lista hardcodeada — regla #35), **logo y PORTADA obligatorios para ABRIR la tienda** (no para aprobar), pipeline sin cuentas borradas, y la portada que "no subía" era HEIC de iPhone → conversión en cliente con `heic-to/csp` (WASM, compatible CSP; heic2any usaba eval y la CSP lo bloqueaba) + errores de upload siempre visibles + límite 10MB alineado. Verificación 21/21. (2) `feat/rediseno-perfil-comercio`: perfil INMERSIVO iterado sobre mockups HTML antes de codear — portada full-bleed con curva austral, buscador SCOPED al comercio (viaja al header con blur al scrollear), sin AppHeader global en `/store/*`, barra "Ver mi pedido", horarios en POPUP centrado con "Abierto hasta HH:MM" real (timezone Ushuaia), card de producto nueva (4 escuelas evaluadas; ganó grilla vidriera: foto ENTERA object-contain, precio 18px NEGRO ⟷ insignia MOOVER con puntos SERVER-SIDE), reseñas en card. **REMOVIDO del perfil público: badge Verificado (×3) y redes sociales** (WhatsApp/IG/FB — fuga de transacciones, criterio ML/PedidosYa; campos en DB para co-marketing) + sección de carga en el panel. Burbuja de soporte SOLO en Mi Perfil. Reglas #40-#41. (3) `fix/pwa-actualizacion-instantanea`: **nunca más "borrá el caché del celular"** — el SW se versiona por build (prebuild estampa hash+timestamp en `sw-version.js` gitignorado), HTML con `cache:no-store` (Safari iOS servía páginas viejas desde su caché heurístico), chequeo de updates al abrir + al volver a primer plano, `updateViaCache:none`. Cada deploy ⇒ banner "Actualizar" en la primera apertura. + Bonus: aviso anti-Self-XSS en consola (estilo Google). Regla #42.

> **Sesión 2026-07-21 → 25 (cortina hub + piloto lead-capture + panel inmediato — resumen)**: la sesión más larga del proyecto. **6 ramas cerradas y deployadas** (batch `-SchemaOnly` + env Telegram en VPS) + **1 rama ABIERTA lista para cerrar**. (1) `feat/rediseno-registro-comercio-repartidor`: la cortina es un HUB "elegí tu mundo" (comercio/repartidor/moover) MONOBRAND rojo + neutros (se probó azul/verde por portal y se descartó), slogan "TODO SE MUEVE", estrategia PILOTO = lead-capture (NO auto-registración; PreLaunchLead + rubro/businessName; plan canónico en `.claude/PLAN-CRECIMIENTO.md`), kickers "COMERCIOS/REPARTIDORES/MOOVERS FUNDADORES", validación propia en forms (sin globos del navegador). (2) `fix/lista-espera-cliente-y-compartir`: rol CLIENTE bien etiquetado en OPS + botón "Compartí Moovy" post-registro. (3) `feat/preguntas-lead-repartidor`: paso 2 opcional post-envío (vehículo / ¿otra app? / rango por viaje — inteligencia de mercado). (4) `fix/copy-legal-prelanzamiento`: números con condición + vigencia (Ley 24.240 art. 8), sin comparativas, finalidad de datos + canal real de borrado (somosmoovy@gmail.com), regla #34. (5) `feat/notificacion-telegram-leads`: Telegram al founder (alta + diff de re-anotación) + email de confirmación al lead por rol (EMAIL_REGISTRY #320-322); "¡Ya estabas anotado!" en re-anotación; hint WhatsApp sin 0 ni 15. (6) `chore/limpieza-imagenes-cortina`. (7) **`feat/panel-inmediato-comercio` (ABIERTA, código completo, `.commit-message` listo)**: modelo Shopify — el comercio entra a su panel al registrarse (auto-login), dashboard DOS MODOS (guía de armado 5 pasos vs operación), barra de continuidad EN VIVO vía `/api/merchant/setup` (client: los layouts de Next no se re-renderizan al navegar), **MES GRATIS DESDE LA APROBACIÓN** (`firstMonthFreeBaseDate` — CAMBIO DE BIBLIA), soporte mobile rediseñado (+ fix: offline bloqueaba crear consulta), Mi Comercio pulido (redes unificadas al popup, horarios con confirmación explícita, Pausar Tienda gris si pendiente, mín 30 chars descripción), nav mobile flotante + header liviano, docs configurables confirmados (habilitación municipal y reg. sanitario OFF por decisión founder tras investigación legal — PedidosYa tampoco los pide).
...
-2. **Verificación post-deploy del batch 07-26** (3 ramas, `-NoDB`): (a) cortina intacta en incógnito; (b) perfil de Pixel Point completo — portada full-bleed, buscador, popup horarios, card con estrella MOOVER, "Ver mi pedido"; (c) **subir la portada HEIC real** desde el panel (prueba de fuego del fix de uploads); (d) pipeline OPS sin el purgado en "Rechazados"; (e) home con header normal + burbuja solo en Mi Perfil; (f) **iPhone del founder**: abrir Moovy con la versión vieja clavada → debe aparecer el banner "Nueva versión disponible" → Actualizar (puede necesitar 2ª apertura la primera vez); (g) consola en prod muestra el "¡FRENÁ!" anti-Self-XSS. Cargarle 5-6 productos a Pixel Point para juzgar la grilla con datos reales.

-1. **Cerrar y deployar `feat/panel-inmediato-comercio`** (HECHO 07-25 — dejar de referenciar) (rama ABIERTA, `.commit-message` listo, SIN schema): en Windows correr `npx tsx scripts/test-first-month-free.ts` (mes gratis desde aprobación — dinero) + `npx tsx scripts/verify-panel-inmediato.ts` + probar flujo completo (registro → panel con guía → docs → barra "todo listo 24-48hs" → aprobar desde OPS → email → modo operación) → `.\scripts\finish.ps1` → `.\scripts\devmain.ps1 -NoDB`. Borrar el comercio de prueba "Pixel Point" (maugrod@gmail.com) y los leads de prueba de OPS después.
-0.5. **Founder (2 min)**: decidir el flag `merchant.doc.bank-account` en `/ops/feature-flags` (recomendación: OFF — con el split MP el comercio cobra directo en su cuenta de MercadoPago y el CBU no se usa para nada; reversible al instante). Docs quedarían en el mínimo profesional: CUIT + Constancia AFIP. (Sentry local: ✅ resuelto por el founder 07-25.)
