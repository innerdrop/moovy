        ...
        <div className="flex flex-col h-[calc(100vh-180px)]">
            ...
            <div className="flex items-center gap-3 pb-4 border-b">
                ...
                <div className="flex-1">
                    <p className="font-semibold text-gray-900 text-sm">{selectedChat.subject}</p>
                    {selectedChat.operator && (
                        <p className="text-xs text-gray-400">Operador: {selectedChat.operator.displayName}</p>
                    )}
                </div>
                <span className={`px-2 py-1 rounded text-xs font-medium ${selectedChat.status === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                    {selectedChat.status === "active" ? "Activo" : selectedChat.status === "waiting" ? "En espera" : "Cerrado"}
                </span>
            </div>
