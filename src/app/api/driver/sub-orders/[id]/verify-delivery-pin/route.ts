// API Route: Driver verifica el PIN de entrega al comprador (SubOrder multi-vendor)
// POST /api/driver/sub-orders/[id]/verify-delivery-pin
...
const pinLogger = logger.child({ context: "verify-delivery-pin-sub" });

export async function POST(
    request: Request,
    { params }: { params: Promise<{ id: string }> }
) {
    const limited = await applyRateLimit(request, "pin:verify-delivery-sub", 5, 10 * 60 * 1000);
    ...
    try {
        ...
        pinLogger.error({ error }, "Error verifying delivery PIN (sub-order)");
        return NextResponse.json(
