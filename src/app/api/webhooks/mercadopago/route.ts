...
export async function POST(request: NextRequest) {
    try {
        ...
        if (paymentStatus === "approved") {
            // Helper canónico — comparte side effects con el polling de /api/payments/[orderId]/status.
            // Idempotente: si el polling ya confirmó (race condition), retorna alreadyConfirmed sin doblar.
            const confirmResult = await confirmOrderPaymentFromMp(order.id, { mpPaymentRaw: mpPayment });
            if (confirmResult.failed) {
                paymentLogger.error(
                    { orderId: order.id, reason: confirmResult.reason },
                    "[webhook-mp] confirm helper reported failure"
                );
            }
...
// ─── Approved Payment ────────────────────────────────────────────────────────
// La rama fix/mp-return-confirmacion (2026-04-26) consolidó la lógica de approval
// en el helper canónico `confirmOrderPaymentFromMp` (ver src/lib/order-payment-confirm.ts).
// El path "approved" ahora llama directo al helper desde el POST principal.
//
// Estructura previa: handleApproved() local + emitPaymentEvent(). Ahora el helper
// emite a los mismos rooms (merchant/seller/admin/order) y manda el email igual.
