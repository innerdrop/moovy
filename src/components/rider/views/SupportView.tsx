...
export default function SupportView({ onBack, onChatRead }: SupportViewProps) {
    ...
    if (loading) {
        return (
            <div className="absolute inset-0 z-50 bg-gray-50 dark:bg-[#0f1117] flex items-center justify-center">
    ...
    return (
        <div className="absolute inset-0 z-50 bg-gray-50 dark:bg-[#0f1117] animate-in slide-in-from-bottom-10 fade-in duration-300 flex flex-col">
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="bg-white dark:bg-[#1a1d27] shadow-sm h-full flex overflow-hidden">
                    {/* Sidebar - Chat List */}
                    <div className={`w-full md:w-72 border-r dark:border-white/10 flex flex-col ${selectedChat || showNewChat ? 'hidden md:flex' : 'flex'}`}>
                        ...
                        <div className="flex-1 overflow-y-auto moovy-pad-nav">
                            {chats.length === 0 ? (
                                ...
                                chats.map((chat) => (
                                    <button
                                        ...
                                        className={`w-full p-3 border-b dark:border-white/10 text-left hover:bg-gray-50 dark:hover:bg-[#22252f] ${selectedChat?.id === chat.id ? 'bg-red-50 dark:bg-red-500/10 border-l-4 border-l-[#e60012]' : ''
                                            }`}
                                    >
                        ...
                        {showNewChat ? (
                            <div className="flex-1 flex flex-col">
                                <div className="p-4 border-b dark:border-white/10 flex items-center gap-3">
                                    <button onClick={() => setShowNewChat(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-[#22252f] rounded-lg">
                                        <ChevronLeft className="w-5 h-5 dark:text-white" />
                                    </button>
                                    <h3 className="font-bold text-gray-900 dark:text-white">Nueva consulta</h3>
                                </div>
                                <form onSubmit={createNewChat} className="flex-1 p-4 flex flex-col">
                                    <input
                                        ...
                                        className="w-full px-3 py-2 border dark:bg-[#22252f] dark:border-white/10 dark:text-white rounded-lg mb-3 focus:outline-none focus:ring-2 focus:ring-[#e60012]"
                                        placeholder="Asunto (opcional)"
                                    />
                                    <textarea
                                        value={newChatMessage}
                                        onChange={(e) => setNewChatMessage(e.target.value)}
                                        className="w-full flex-1 resize-none mb-3 px-3 py-2 border dark:bg-[#22252f] dark:border-white/10 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#e60012]"
                            ...
                            <>
                                <div className="p-4 border-b dark:border-white/10 flex items-center gap-3 bg-gray-50 dark:bg-[#22252f]">
                                    <button onClick={() => setSelectedChat(null)} className="md:hidden p-2 hover:bg-gray-100 dark:hover:bg-[#2a2d38] rounded-lg">
                                        <ChevronLeft className="w-5 h-5 dark:text-white" />
                                    </button>
                                    <div className="flex-1 pt-safe-top">
                                        <h3 className="font-bold text-gray-900 dark:text-white">{selectedChat.subject}</h3>
                                        <p className="text-xs text-gray-500 dark:text-gray-400">Soporte MOOVY</p>
                                    </div>
                                </div>

                                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-100 dark:bg-[#0f1117] moovy-pad-nav">
                                    {selectedChat.messages.map((msg) => (
                                        <div key={msg.id} className={`flex ${msg.isFromAdmin ? 'justify-start' : 'justify-end'}`}>
                                            <div className={`max-w-[85%] rounded-2xl px-4 py-2 ${msg.isFromAdmin ? 'bg-white dark:bg-[#1a1d27] shadow-sm' : 'bg-[#e60012] text-white'
                                                }`}>
                                                {msg.isFromAdmin && (
                                                    <p className="text-xs font-medium text-[#e60012] mb-1">MOOVY</p>
                                                )}
                                                <p className={`text-sm ${msg.isFromAdmin ? 'dark:text-white' : ''}`}>{msg.content}</p>
                                                <div className={`flex items-center gap-1 mt-1 text-xs ${msg.isFromAdmin ? 'text-gray-400 dark:text-gray-500' : 'text-white/70'}`}>
                                ...
                                <div className="p-4 bg-white dark:bg-[#1a1d27] border-t dark:border-white/10 border-gray-100" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
                                    <form onSubmit={sendMessage} className="flex gap-2 items-center bg-gray-50 dark:bg-[#22252f] rounded-[24px] p-2 border border-transparent dark:border-white/10 focus-within:bg-white dark:focus-within:bg-[#2a2d38] focus-within:border-red-100 dark:focus-within:border-white/20 focus-within:shadow-[0_4px_20px_-4px_rgba(124,58,237,0.1)] transition-all duration-300">
                                        <input
                                            ...
                                            className="flex-1 bg-transparent px-4 py-2 outline-none text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 font-medium"
                                        />
                                        <button
                                            type="submit"
                            ...
                            <div className="flex-1 flex items-center justify-center text-center p-8 hidden md:flex">
                                <div>
                                    ...
                                    <h3 className="font-bold text-gray-900 dark:text-white mb-2">Soporte</h3>
                                    <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">¿Tenés dudas? Escribinos.</p>
