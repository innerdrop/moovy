// Diagnóstico completo de login: simula authorize() y muestra config NextAuth
...
async function testLogin() {
    ...
    const password = process.argv[3]
        || process.env.OPS_LOGIN_PASSWORD
    ...
    if (user.deletedAt) {
        ...
        console.log("   Fix: npx tsx scripts/create-admin.ts", email);
        await prisma.$disconnect();
        return;
    }
    ...
    if (!process.env.AUTH_SECRET && !process.env.NEXTAUTH_SECRET) {
        console.log("🚨 PROBLEMA: Ni AUTH_SECRET ni NEXTAUTH_SECRET están definidos.");
    } else if (!process.env.AUTH_TRUST_HOST && !process.env.NEXTAUTH_URL) {
        console.log("🚨 PROBLEMA: Falta AUTH_TRUST_HOST=true o NEXTAUTH_URL.");
    } else {
        console.log("✅ Config parece correcta.");
    }
