# MOOVY
Última actualización: 2026-04-20 (rama `fix/ux-pulido-pre-launch` RESUELTA — 10 issues de UX pulido pre-launch: ISSUE-036 diversidad home (mostOrdered→bestRated→newMerchants disjuntos, hide rows <2, hide all si <3 merchants), 037 contador unificado onboarding, 038 chip tri-estado comercio (Pendiente/Cerrada/Abierta con subtítulo), 039 helper `formatARS` + barrido driver portal + FavoritesCarousel, 041+042 chip comercio + breadcrumbs en detalle producto, 043 quitar line-through verde (→ opacity-70), 044 modal custom eliminar dirección con bloqueo single-address, 047 semántica "Abierta/Cerrada a ventas" en portal vendedor, 059 desglose envío siempre visible en checkout con método. Cleanup extra: 5 archivos con NUL bytes corruptos al final (addresses/route.ts, mp-reconcile/route.ts, ops/soporte/page.tsx, not-found.tsx, useRealtimeOrders.ts) — tsc ahora limpio sobre todo src/.)
