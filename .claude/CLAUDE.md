# MOOVY
Última actualización: 2026-04-20 (rama `feat/ux-flujo-checkout-y-onboarding` RESUELTA — 2 issues con cambio financiero real: ISSUE-020 mes 1 gratis (0% comisión 30 días corridos desde `Merchant.createdAt`) + ISSUE-045 progress bar puntos + bloque explicativo. **ISSUE-020**: 3 helpers nuevos en `src/lib/merchant-loyalty.ts` (`isInFirstMonthFree`, `getFirstMonthFreeEndDate`, `getFirstMonthFreeDaysRemaining`) + constante `FIRST_MONTH_FREE_DAYS=30`. `getEffectiveCommission()` ahora aplica precedencia `commissionOverride > first-month-free (0%) > tier > fallback 8%`. Fix bug pre-existente crítico: path multi-vendor de `/api/orders/route.ts` leía `merchant.commissionRate` crudo en vez de `getEffectiveCommission()` (bypassaba mes gratis y lealtad). Single-vendor cambiado de `||` a `??` para que `0` no caiga al fallback 8%. Banner esmeralda en dashboard merchant (`/comercios/(protected)/page.tsx`), promesa en `/comercios/pendiente-aprobacion`, widget con banner forward-compat. Script `scripts/test-first-month-free.ts` con 12+ asserts (funciones puras + integración Prisma real). **ISSUE-045**: `/api/points/route.ts` expone `userLevel` vía `getUserLevel(userId)` de `src/lib/points.ts` (Biblia v3: MOOVER/SILVER/GOLD/BLACK por pedidos DELIVERED últimos 90 días). `/puntos/page.tsx` Hero Card con ícono dinámico + nombre del nivel real (no más hardcodeado) + progress bar dorada hacia el siguiente nivel (o "Nivel máximo" si BLACK). Bloque destacado "Cómo funcionan tus puntos" con 3 columnas (Valor 1pt=$1 / Mínimo 500 / Máximo 20% subtotal) leyendo `pointsConfig` del backend. Pie dinámico: "Ya podés canjear" si `balance >= minPointsToRedeem`, si no "Te faltan N pts". Accesibilidad: role=progressbar + aria-*. **Diferidos a próxima rama**: ISSUE-055+056 (breadcrumb checkout 3 tabs Entrega→Pago→Confirmar + mover tipo entrega), ISSUE-021 (tour buyer primera vez).)
