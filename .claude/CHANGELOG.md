...
El bug reportado (la barra de "Agregar al carrito" escondida detrás de la
navegación en la app instalada y pegada a ella en el navegador) no era de esa
pantalla: eran 10 barras con el offset escrito a mano. Y las dos que ya se
habían arreglado a mano estaban mal igual — el "+72px" no contaba el voladizo
del botón MOOVER.

Motor
- globals.css: --moovy-bar-bottom declarada COMPLETA por zona (comprador,
  comercio, vendedor, repartidor, ops). No se compone desde :root: el navegador
  sustituye las var() antes de heredar, así que componer no habría hecho nada
  fuera del camino del comprador.
- src/lib/useNavPeak.ts: mide el alto visual real de la navegación en el
  dispositivo (con data-nav-peak para los hijos que sobresalen). Cubre el
  escalado de fuente del sistema Android y cualquier rediseño futuro. Si no
  corre el JS, el CSS ya trae valores por zona.
- .moovy-pad-bar / .moovy-pad-nav: el colchón del contenido sale del mismo
  token, con scroll-padding-bottom para el scroll automático al enfocar un campo.

Las 10 barras migradas
agregar al carrito · ir a pagar · detalle de pedido (+ skeleton) · "Tenés
cambios sin guardar" x4 · selección múltiple · carrito de paquetes · volver
arriba · soporte del repartidor. Más las dos que ya estaban "arregladas"
(perfil de comercio y botón flotante del carrito), que tenían el número mal.

La burbuja de soporte (11a barra, encontrada al verificar en pantalla)
Tenía BOTTOM_NAV_SPACE = 116 escrito a mano — la TERCERA reaparición del mismo
bug, esta vez en JavaScript, donde el chequeo de className no lo veía. El 116 se
quedaba corto en cuanto la pantalla tenía además una barra de acción: la burbuja
se le sentaba encima. Ahora usa medirEspacioInferior(), que mide la barra real si
hay una y si no resuelve el token. check:barras ahora también atrapa este patrón.

Verificación de la geometría (Chromium + insets de safe-area inyectados por CDP)
caja de la nav 64px · voladizo del botón MOOVER 17-18px · punta real a 81-82px.
El "+72" que teníamos se quedaba ~10px corto y le pasaba por encima al botón.
Hueco final barra->punta: 16px, el número que pide Material 3.
Confirmado en el equipo del founder: caja 64px, punta 94.1px con colchón 0.

Android (la mayoría del parque en Argentina)
- Piso max(12px, ...) en vendedor, repartidor y OPS: env(safe-area-inset-bottom)
  vale 0 en Android y en toda PWA instalada en Android, así que la fila de
  botones caía donde el sistema dibuja la barra de gestos y se comía el toque.
- Fuera el md:bottom-6 de la selección múltiple: la navegación se oculta recién
  en lg (1024px), así que estaba roto en toda tablet.
- Fuera el doble backdrop-blur de las barras de acción (patrón MercadoLibre / X).
- El skeleton de la navegación copia la geometría de la píldora real.
- Las animaciones slide-in pasan a fundido de opacidad (regla #12).

Producto (patrón Rappi / PedidosYa, aprobado por el founder)
La navegación desaparece en carrito, checkout y detalle de producto. Nunca
conviven dos barras apiladas. Las tres conservan salida visible propia.

Copy: Moovy no es una app de comida (regla #46)
- Fuera las palabras clave gastronómicas que ordenaban el home: dejaban a una
  ferretería última a cualquier hora. Ahora: abiertos → puntuación → demora.
- Dos frases por franja horaria, alternando cada 8s con fundido de opacidad.
- Placeholder del buscador rotando entre rubros (hero + header).
- Landing y perfil del repartidor.

Herramientas
- npm run check:barras (nuevo): falla si vuelve a aparecer un offset a mano.
  Encontró una décima barra que la auditoría manual se había salteado.
- /debug/viewport: muestra los valores reales del teléfono. El emulador de
  Chrome no simula env(safe-area-inset-*).
- check:parse y db:extensiones repuestos en package.json (los scripts existían
  pero las entradas se habían perdido en un merge).

De paso: test inestable por reloj
"should accept scheduled delivery during valid business hours" programaba HOY a
las 15:00 y el schema exige 1.5 h de anticipación, así que fallaba en cualquier
corrida entre las 13:30 y las 15:00. Ahora programa siempre mañana a las 15:00,
que cae entre 15 y 39 h por delante a cualquier hora del día.

Queda abierto: el teclado tapa las barras fijas (Chrome 108+ y iOS). Rama propia.

**Archivos:** .claude/CLAUDE.md, ISSUES.md, _to_delete/_tmp_CHANGELOG.md, _to_delete/_tmp_CLAUDE.md, package.json, scripts/check-barras.mjs, src/__tests__/orders.test.ts, src/app/(store)/carrito/page.tsx (+32 mas)

## 2026-07-29 (rama `feat/barras-flotantes-y-copy`)

fix: las barras inferiores dejan de esconderse detrás de la navegación, en todo el sitio

El founder mandó cuatro capturas: la barra de "Agregar al carrito" quedaba **escondida detrás**
de la navegación con Moovy instalada como app, y **pegada** a ella en el navegador. Después
preguntó algo que cambió el alcance de la rama: *"siempre hablás de iPhone, pero en Argentina
la mayoría usa Android, con teléfonos de todos los tamaños"*.

Tenía razón, y el problema era peor de lo diagnosticado.

### Lo que estaba mal

**No era un bug de la página de producto: eran 10 barras.** Cada una con el offset escrito a
mano — `bottom-16`, `bottom-24`, `bottom-0`, `bottom-16 md:bottom-6`, y hasta un
`calc(6rem + env(safe-area-inset-bottom) + 1rem)`. Y las **dos que ya se habían arreglado a
mano** en ramas anteriores estaban **mal igual**: el `+72px` sumaba los 62px de la caja de la
píldora, pero el botón MOOVER sobresale por encima con `-top-3`, así que el hueco real era de un
par de píxeles, no de 10. Ese número mal calculado se había copiado de un archivo a otro.

**En Android, el colchón de safe-area no existe.** `env(safe-area-inset-bottom)` devuelve **0**
en la enorme mayoría de los Android — y en **toda** PWA instalada en Android. Los 34px son el
caso iPhone. O sea que todo el mecanismo de safe-area no hacía nada justo donde el founder vio
el bug. Peor: en el panel del vendedor, el del repartidor y OPS, la fila de botones quedaba
exactamente donde Android dibuja la barra de gestos, y **el sistema se comía el toque**.

**El número no se podía calcular.** Un panel de cuatro especialistas dio cuatro respuestas
distintas para el mismo hueco (72, 81, 83 y 84) leyendo el mismo código, porque depende del alto
de línea real de una etiqueta de 10px con la fuente ya cargada.

### Lo que se hizo

**Un token por zona, declarado completo.** `--moovy-bar-bottom` se define en `globals.css`, una
vez por zona (comprador, comercio, vendedor, repartidor, ops). Cada zona **re-declara la
variable entera** en vez de componerla desde `:root`: componer no funciona, porque el navegador
sustituye las `var()` **antes** de heredar (`css-variables-1`). Es el error que iban a cometer
tres de los cuatro informes del panel y que detectaron los revisores adversariales.

**`useNavPeak` mide la navegación en el dispositivo** y publica el resultado, marcando con
`data-nav-peak` a los hijos que sobresalen de la caja. Cubre gratis el escalado de fuente del
sistema Android (desde Chrome 113 funciona como zoom de página; ~40% de los usuarios Android no
está en el valor por defecto) y cualquier rediseño futuro. Si el JS no corre, el CSS ya trae
valores por zona.

**La navegación desaparece en las pantallas de conversión** (carrito, checkout, detalle de
producto). Patrón Rappi / PedidosYa: nunca conviven dos barras fijas apiladas, así que no hay
offset que calcular, y en un 360×720 se recuperan ~100px justo en la pantalla que convierte. Se
verificó que las tres conserven salida visible propia — en standalone no hay botón "atrás".

**Piso `max(12px, ...)`** en vendedor, repartidor y OPS. **Fuera el `md:bottom-6`** de la
selección múltiple (estaba roto en toda tablet, porque la navegación se oculta recién en `lg`).
**Fuera el doble `backdrop-blur`** de las barras de acción (patrón MercadoLibre / X: su parque es
Galaxy A y Moto G). **El skeleton de la navegación copia la geometría de la píldora real**, para
que no salte al terminar de cargar. Las animaciones `slide-in` de los banners "Tenés cambios sin
guardar" pasaron a fundido de opacidad: el transform residual era exactamente la causa del texto
borroso de la regla #12.

**`npm run check:barras`** falla si vuelve a aparecer un offset escrito a mano. Encontró una
décima barra que la auditoría manual se había salteado (el soporte del repartidor).

**`/debug/viewport`** muestra los valores reales del teléfono y dibuja una barra de prueba a la
altura calculada. Existe porque el emulador de Chrome **no simula** `env(safe-area-inset-*)`
(bug de Chromium 40718410): sin teléfonos reales estaríamos adivinando de nuevo.

### Copy: Moovy no es una app de comida (regla #46)

El home ordenaba los comercios con listas de palabras gastronómicas por franja horaria —
"pizza", "sushi", "parrilla", "rotisería". Una ferretería no tiene ninguna, así que quedaba
**última a cualquier hora del día**. Se reemplazó por: abiertos primero, después mejor puntuados,
después los que llegan más rápido. Sin rubros escritos a mano.

Las frases del hero pasaron a **dos por franja**, alternando cada 8 segundos con fundido de
opacidad (y quietas si el usuario pidió reducir movimiento). Los saludos se quedaron: nunca
fueron el problema. El placeholder del buscador **rota entre rubros** — "pizza", "tornillos",
"pañales", "ibuprofeno", "cuaderno rayado" — en el hero y en el header; antes decía "¿Qué querés
pedir?", y "pedir" suena a delivery de comida. Copy corregido también en la landing y en el
perfil del repartidor.

### De paso

`check:parse` y `db:extensiones` faltaban en `package.json`. Los scripts existían en `scripts/`
desde `feat/busqueda-inteligente`, pero las entradas se habían perdido en algún merge — o sea
que `npm run check:parse` venía fallando.

### Queda abierto

**El teclado tapa las barras fijas**, en Android e iOS por igual (desde Chrome 108 el default es
`resizes-visual`). No entró acá a propósito: las soluciones candidatas se contradicen entre sí y
el detector por `visualViewport` que proponen **confunde el pinch-zoom con el teclado** — al
hacer zoom para leer, la navegación desaparecía. Necesita rama propia.
...
**Archivos:** scripts/seed-biblia-launch.ts, src/app/(store)/puntos/page.tsx, src/components/puntos/RewardsVitrina.tsx

## 2026-07-14 (rama `feat/moover-canje-recompensas`)

feat: canje de puntos por recompensas de UN TOQUE en el checkout (sin código). Requiere prisma db push. El cliente elige una recompensa con un toque en el checkout y se aplica al instante — nunca escribe un código, no hay billetera ni vencimiento. SCHEMA (aditivo): modelo Reward (catálogo: label/icon/pointsCost/type/value/isActive/order). APIs: /api/ops/rewards (+[id]) CRUD admin con Zod, y /api/rewards público (activas, tolera tabla ausente). MONEY (orders/route.ts): nuevo bloque server-side AUTORITATIVO — si viene rewardId (y no se usó el slider), valida reward activa + saldo ≥ pointsCost y resuelve el beneficio como un descuento + gasto de pointsCost puntos, reusando el REDEEM existente. FREE_DELIVERY = descuento del envío (el cliente paga $0 de envío pero el repartidor cobra igual desde el snapshot inmutable → lo absorbe Moovy); FIXED_AMOUNT = min(value, subtotal). El total nunca queda negativo. UI: componente RewardPicker (botones de un toque, deshabilita los inasequibles/no-aplicables, mutuamente exclusivo con el slider de puntos) en el checkout; rewardId se manda en el POST del pedido. OPS: página /ops/recompensas (CRUD con modal) + item en el sidebar (Marketing). Script scripts/verify-recompensas.ts (catálogo bien formado + sin pedidos con total negativo). PENDIENTE menor: vidriera aspiracional en /puntos (opcional). ACCIÓN OPS: crear las recompensas en /ops/recompensas (el catálogo arranca vacío).

**Archivos:** prisma/schema.prisma, scripts/verify-recompensas.ts, src/app/(store)/checkout/page.tsx, src/app/api/ops/rewards/[id]/route.ts, src/app/api/ops/rewards/route.ts, src/app/api/orders/route.ts, src/app/api/rewards/route.ts, src/app/ops/(protected)/recompensas/page.tsx (+3 mas)

## 2026-07-14 (rama `feat/moover-referido-residual`)

feat: referido residual de por vida (MOOVER). Requiere prisma db push (cambios aditivos). El referidor gana un residual por cada N pedidos ENTREGADOS de su referido, para siempre, un solo nivel — atado a pedidos reales pagados, así se autofinancia y no se puede duplicar. SCHEMA (aditivo): Referral.residualsPaid (contador de hitos ya pagados) + PointsConfig.referralResidualBonus (default 1000) + referralResidualEvery (default 10). LÓGICA: nueva función processReferralResidual(refereeUserId) en points.ts — busca el Referral del user, cuenta sus pedidos DELIVERED, calcula hitos = floor(pedidos / every) y, si hay hitos nuevos vs residualsPaid, acredita (hitosNuevos × bonus) al referidor dentro de una tx Serializable que actualiza el contador (idempotente + race-safe, retry ante conflicto). Se engancha en awardOrderPointsIfDelivered (tras activar los bonos), fire-and-forget (no rompe el flujo del pedido). Config expuesto en points.ts + ops-config.ts con 2 inputs nuevos en /ops/config-biblia (monto + cada cuántos pedidos). Texto del residual sumado a la tarjeta de referido en /puntos ("+1.000 pts cada 10 pedidos de tu amigo, de por vida"). Script scripts/verify-referido-residual.ts (config + invariante anti-sobrepago: residualsPaid <= hitos reales, contra la DB). Los nuevos campos de config traen su default (1000/10) al hacer db push, así que no requiere acción manual en OPS.

**Archivos:** prisma/schema.prisma, scripts/verify-referido-residual.ts, src/app/(store)/puntos/page.tsx, src/app/ops/(protected)/config-biblia/BibliaConfigClient.tsx, src/lib/ops-config.ts, src/lib/points.ts

## 2026-07-14 (rama `feat/moover-bono-resena`)

feat: bono por reseña MOOVER (feature nueva, sin cambios de schema). Reactiva el reviewBonus (la columna ya existía en la DB, estaba dormida): re-expuesto en points.ts (interface + defaultConfig 1000 + getPointsConfig + updatePointsConfig) y en ops-config.ts (tipo + load + save), con input "Bono reseña" en /ops/config-biblia. Nueva función awardReviewBonus(userId, orderId) que otorga el bono UNA sola vez por pedido — idempotente dentro de una tx Serializable (chequeo del tx REVIEW previo por orderId + retry ante conflicto de serialización), aunque el user califique comercio + repartidor + vendedor por separado. Cableado en los 3 endpoints de rating (rate = driver, rate-merchant, rate-seller): tras guardar la calificación se otorga el bono y se devuelve pointsEarned en la respuesta (mensaje "Ganaste X puntos"). Se vuelve a mostrar la tarjeta "Por cada reseña — 1.000 pts" en /puntos. Script scripts/verify-review-bonus.ts (config expuesto + sin bonos duplicados por pedido). NOTA OPS: la fila viva de PointsConfig tiene reviewBonus=25 (default viejo del schema) — setear "Bono reseña = 1000" en /ops/config-biblia para que otorgue $1.000.

**Archivos:** scripts/verify-review-bonus.ts, src/app/(store)/puntos/page.tsx, src/app/api/orders/[id]/rate-merchant/route.ts, src/app/api/orders/[id]/rate-seller/route.ts, src/app/api/orders/[id]/rate/route.ts, src/app/ops/(protected)/config-biblia/BibliaConfigClient.tsx, src/lib/ops-config.ts, src/lib/points.ts

## 2026-07-14 (rama `fix/moover-valor-punto-checkout`)

fix: alinear MOOVER y todo el contenido a las decisiones de la Biblia v5 (sin cambios de schema). (1) BUG DE DINERO: el PointsWidget del checkout usaba 1 pt = $0,015 y tope 15% hardcodeados (distinto de lo que cobra el servidor); ahora lee el config real (/api/points?config=true → 1 pt = $1, tope maxDiscountPercent) y muestra EXACTAMENTE lo que se descuenta. (2) NIVELES: defaults 3/10/22 (antes 5/15/40, BLACK inalcanzable) y ahora editables desde OPS vía PointsConfig.tierConfigJson (resolveLevelConfigs). (3) points.ts defaults v5 (maxDiscount 50, signup 2500, referido 3500/2500). (4) TÉRMINOS DEL COMPRADOR: fuera el efectivo, fuera Rappi/PedidosYa/Glovo, corregido el split (repartidor 80%), canje 50%. (5) COMISIONES: eliminada la "Tarifa Fundador 5%" (solo 30 días gratis + 10%), valor del punto $0,01→$1, referido unificado a 3.500/2.500. (6) "desde 8%" → 10% en /comercio/info, MerchantFAQ y StatsCounter. (7) /puntos, /moover, terminos-moover, ayuda, invitar: bienvenida $2.500 (igual para todos, sin mínimo de $5.000), canje 50%, niveles 3/10/22, referido 3.500/2.500 (sin mínimo $8.000), "no vencen mientras uses Moovy", y se quitó el texto de features NO construidas (reseña, residual, canje por producto, desafíos). (8) EMAILS: canje 50%, email de "puntos por vencer" reescrito para no amenazar vencimiento, referido 3.500, y se quitó la promesa de efectivo al repartidor. (9) Efectivo quitado de la página de producto y de cancelaciones. (10) Borrados 4 componentes muertos con copy prohibido (HowItWorks, TrustBar, SpecialOffers, PromotionalPopup). NOTA OPS: en /ops/config-biblia setear maxDiscountPercent=50, signupBonus=2500, referralBonus=3500, refereeBonus=2500. Decisión pendiente: mínimos anti-abuso de bienvenida/referido (minPurchaseForBonus, minReferralPurchase).

**Archivos:** docs/MOOVY_Plan_Negocios_Biblia_v5.docx, docs/MOOVY_Simulador_Crecimiento.xlsx, src/app/(store)/ayuda/page.tsx, src/app/(store)/mi-perfil/invitar/page.tsx, src/app/(store)/productos/[slug]/ProductDetailClient.tsx, src/app/(store)/puntos/page.tsx, src/app/cancelaciones/page.tsx, src/app/comercio/info/page.tsx (+16 mas)

## 2026-07-13 (rama `feat/home-builder-pagina-inicio`)

feat: Home Builder — organizador de secciones del home desde OPS. Nuevo modelo HomeSection (key/label/order/enabled) + registry canónico src/lib/home/sections.ts (fuente de verdad de las keys válidas + resolveHomeLayout). La home (src/app/(store)/page.tsx) deja el orden clavado en el código y renderiza las secciones MOVIBLES según order/enabled de la DB, con Hero + "Abiertos ahora" y CTAs + Footer FIJOS; cada sección conserva su guard de "hay datos" (el enabled de OPS es un candado adicional) y hay fallback al orden por defecto del registry si la tabla está vacía. API admin /api/ops/home-sections (GET siembra idempotente las filas faltantes, PUT valida keys contra el registry + upsert en tx Serializable). Página OPS /ops/pagina-inicio: arrastrar-para-reordenar + flechas + interruptor visible/oculta + accesos directos a categorías/hero/banners/destacados, con estados loading/error/guardado. Item nuevo en el sidebar (grupo Marketing). Script scripts/verify-home-sections.ts. Requiere prisma db push (tabla HomeSection) — deploy MODO SCHEMA, no -NoDB.

**Archivos:** .claude/CLAUDE.md, ISSUES.md, prisma/schema.prisma, scripts/verify-home-sections.ts, src/app/(store)/page.tsx, src/app/api/ops/home-sections/route.ts, src/app/ops/(protected)/pagina-inicio/page.tsx, src/components/ops/OpsSidebar.tsx (+1 mas)
...
**Archivos:** src/app/api/mp/callback/route.ts, src/app/comercios/(protected)/OnboardingChecklist.tsx, src/app/comercios/(protected)/configuracion/page.tsx, src/app/comercios/(protected)/layout.tsx, src/app/comercios/(protected)/mi-comercio/_data.ts, src/app/comercios/(protected)/mi-comercio/_ui.tsx, src/app/comercios/(protected)/mi-comercio/documentacion/page.tsx, src/app/comercios/(protected)/mi-comercio/entregas/page.tsx (+15 mas)

## 2026-07-09 (rama `feat/bloquear-publicidad`)

feat: la sección **Publicidad** ahora se bloquea igual que Paquetes cuando su flag (`merchant.publicidad`) está OFF — se muestra en gris con candado y no navega, en vez de ser un link que abría la página con el mensaje "no disponible". Antes: desktop la dejaba como link normal (sin `requiresFlag`) y mobile la escondía. Ahora: desktop (`layout.tsx`) suma `requiresFlag: "merchant.publicidad"` + `publicidadEnabled` y generaliza el `locked` a cualquier flag OFF (`flagEnabled[item.requiresFlag]`); mobile (`MobileMoreMenu.tsx`) devuelve `"locked"` en vez de `"hidden"` para los items gateados. Sin cambios de schema → deploy `-NoDB`.

**Reorg del panel comercio (mismo branch):** se fusionó **"Ajustes"** dentro de **"Mi Comercio"**, que ahora es un **hub de tarjetas** (Perfil · Horarios y estado · Entregas y pedidos · MercadoPago · Documentación). Cada tarjeta lleva a una sub-página enfocada. Se reusan los formularios existentes con un prop `section` (gating por sección, sin reescribir lógica): `MiComercioForm` (perfil/horarios) y `SettingsForm` (estado/entregas/mercadopago/documentacion). Loaders compartidos en `mi-comercio/_data.ts`, header/error en `_ui.tsx`. Se sacó "Ajustes" del sidebar y del menú mobile; `/comercios/configuracion` quedó como **redirect** al hub (no rompe links viejos). Links de onboarding + callback de MP apuntan directo a la sub-página correcta. Objetivo: navegación simple, cada página hace UNA cosa, sin dashboards complejos.

**Selector de tamaño sin vehículo (mismo branch):** se sacó la línea "Se entrega en: Bici/Moto/..." de cada tarjeta de tamaño. El vehículo es un dato interno (depende de quién esté online + batching + distancia) y encima era por-unidad, así que engañaba (la tarjeta "Pequeño" decía bici, pero 10 unidades no van en bici). Ahora el comercio elige por lo que sí puede juzgar: tamaño/peso de UNA unidad + ejemplos. Texto reencuadrado ("una unidad") + nota tranquilizadora: "si piden varias unidades, calculamos el envío sumando el peso total automáticamente" (el motor ya hace `weightGrams × quantity`, verificado en `product-weight.ts`). En `SizeSelector.tsx` + helper de Nuevo/Editar producto.

**Tamaño OBLIGATORIO (mismo branch):** seleccionar tamaño ahora es requerido, como foto/descripción/precio. En el alta (`NewProductForm`): asterisco + error + scroll a la sección + `isSubmittable` lo incluye (vale tarjeta o gramos en modo avanzado). Server: `createProduct` bloquea sin `sizeSnapshot.weightGrams`; el gate de publicación `toggleProductActive` suma "el tamaño" a lo que falta; el `isComplete` de `updateProduct`, de `ProductsSearchContainer` (chip "Incompleto") y de "Mostrar" en lote (`bulk-actions.ts`) exigen `weightGrams > 0`. Los productos importados (sin tamaño) quedan "Incompletos" hasta que el comercio les asigne uno — no se pueden publicar sin él.

**Fix dirty-state del precio (mismo branch):** en Editar producto, cambiar entre "Precio final" y "Con recargo" activaba el botón Guardar aunque no se hubiera modificado nada (el `isDirty` comparaba el MODO). Ahora el precio ensucia el form solo si cambia el **precio final** (lo que paga el cliente); alternar el modo sin tocar el valor ya no marca cambios. Se sacó del `isDirty` la comparación por modo/base/recargo (y el estado `priceMeta` asociado) en `EditProductForm.tsx`.

**Código de barras + herramientas de lista (mismo branch):** el `barcode` se importaba pero no había dónde verlo/cargarlo. Ahora hay **campo de código de barras** (interno, opcional) en Nuevo y Editar producto, persistido en `createProduct`/`updateProduct` (Zod + manejo de duplicado P2002 con mensaje amable; es único por comercio). La **búsqueda de la lista** ahora matchea también por código (para escanear/tipear y encontrar el producto). NUNCA se muestra en la tienda. Además: **orden** configurable (Recomendado · Precio ↑/↓ · Nombre A-Z · Stock: menos primero · Más nuevos) y **paginación + cantidad por página arriba Y abajo** de la lista. En `ProductsSearchContainer.tsx` + `actions.ts` + ambos forms + `productos/[id]/page.tsx`.

**Archivos:** src/app/comercios/(protected)/layout.tsx, src/components/comercios/MobileMoreMenu.tsx, src/components/comercios/MiComercioForm.tsx, src/components/comercios/SettingsForm.tsx, src/components/comercios/SizeSelector.tsx, src/components/comercios/NewProductForm.tsx, src/components/comercios/EditProductForm.tsx, src/app/comercios/(protected)/mi-comercio/page.tsx, src/app/comercios/(protected)/mi-comercio/_data.ts, src/app/comercios/(protected)/mi-comercio/_ui.tsx, src/app/comercios/(protected)/mi-comercio/{perfil,horarios,entregas,mercadopago,documentacion}/page.tsx, src/app/comercios/(protected)/configuracion/page.tsx, src/app/comercios/(protected)/OnboardingChecklist.tsx, src/app/api/mp/callback/route.ts

## 2026-07-09 (rama `feat/recargo-moovy-y-tamano-toggle`)

feat: recargo Moovy en precio (comision sobre el final, anti-gaming) + toggle de tamano + import trata el precio como precio del local + acciones en lote en la lista de productos (ocultar/mostrar/eliminar/categoria/precio/CSV)

**Archivos:** .claude/CLAUDE.md, prisma/schema.prisma, scripts/verify-recargo-moovy.ts, src/app/api/comercios/products/import/route.ts, src/app/comercios/(protected)/productos/[id]/page.tsx, src/app/comercios/(protected)/productos/importar/page.tsx, src/app/comercios/(protected)/productos/nuevo/page.tsx, src/app/comercios/(protected)/productos/page.tsx (+8 mas)

## 2026-07-09 (rama `feat/recargo-moovy-y-tamano-toggle`)

feat: recargo Moovy en el precio del producto + toggle del selector de tamaño + "guardar solo si hay cambio". **RECARGO**: el comercio publica a su precio de local + un recargo % propio (calculadora), o tipea el precio final directo. Nuevo componente compartido `PriceRecargoField` (Nuevo + Editar) con desglose en vivo (final / comisión / recibís, 0% en primer mes). Dos columnas nuevas en `Product`: `basePrice` + `markupPercent` = metadata NO-cobrable (recuerdan cómo se armó el precio para re-editar). **La comisión SIEMPRE cae sobre el precio FINAL** (`Product.price`); cobrar sobre el local sería gameable a ~$0. Fuente única `derivePricing`/`deriveImportPricing` en `src/lib/finance/product-pricing.ts`; el server re-deriva en modo markup e IGNORA el `price` del cliente (anti-gaming). **IMPORTADOR**: el precio mapeado del CSV es el precio del LOCAL → recargo por lote opcional + escotilla "estos ya son precios finales" (va directo, sin metadata). **TAMAÑO**: `SizeSelector` ahora es toggle — reapretar la card elegida la deselecciona (`onChange(null)`, cae al vehículo por defecto). **GUARDAR**: el banner sale de un diff real contra el snapshot inicial (Edit ya lo hacía; en Nuevo se resuelve con el toggle). Verificación: `scripts/verify-recargo-moovy.ts`. **TOCA SCHEMA** → deploy en modo schema (`devmain.ps1 -SchemaOnly`), no `-NoDB`.

**Acciones en lote (mismo branch):** en la lista de productos el comercio ahora selecciona de a uno / la página / todos los del filtro (banner estilo Gmail) y aplica en masa: **Ocultar**, **Mostrar** (salta incompletos e informa cuántos), **Eliminar** (hard-delete salvo con historial de ventas → se ocultan, preservando pedidos), **Cambiar categoría**, **Ajustar precio ±%** (raw SQL scopeado, limpia la metadata de recargo) y **Exportar CSV** (cliente). Server en `src/app/comercios/bulk-actions.ts`: verifica propiedad de cada id + scope `merchantId` (sin IDOR), Zod, cap 5000. Resuelve la limpieza de la importación (1.100+ borradores). Barra flotante + modales de confirmación en `ProductsSearchContainer`.

**Archivos:** prisma/schema.prisma, src/lib/finance/product-pricing.ts, src/components/comercios/PriceRecargoField.tsx, src/components/comercios/NewProductForm.tsx, src/components/comercios/EditProductForm.tsx, src/components/comercios/SizeSelector.tsx, src/components/comercios/ProductsSearchContainer.tsx, src/app/comercios/actions.ts, src/app/comercios/bulk-actions.ts, src/app/comercios/(protected)/productos/nuevo/page.tsx, src/app/comercios/(protected)/productos/[id]/page.tsx, src/app/comercios/(protected)/productos/page.tsx, src/app/comercios/(protected)/productos/importar/page.tsx, src/app/api/comercios/products/import/route.ts, scripts/verify-recargo-moovy.ts
